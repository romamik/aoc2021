...
impl Matrix {
    ...
    fn rotate90cw(axis: Axis) -> Matrix {
        match axis {
            Axis::X => Matrix([[1, 0, 0], [0, 0, 1], [0, -1, 0]]),
            Axis::Y => Matrix([[0, 0, -1], [0, 1, 0], [1, 0, 0]]),
            Axis::Z => Matrix([[0, 1, 0], [-1, 0, 0], [0, 0, 1]]),
        }
    }
    ...
    fn mul(&self, right: &Matrix) -> Matrix {
        ...
        let v0 = self.apply(&Vector([b[0][0], b[1][0], b[2][0]])).0;
        let v1 = self.apply(&Vector([b[0][1], b[1][1], b[2][1]])).0;
        let v2 = self.apply(&Vector([b[0][2], b[1][2], b[2][2]])).0;
...
fn test1(test1_input: &Input) {
    ...
    let scanners = (0..=4).map(|n| test1_input.get(&format!("scanner {}", n)).unwrap()).collect::<Vec<_>>();

    // try align scanners 0 and 1
    let align_result = try_align(scanners[0], scanners[1], MIN_PAIR).unwrap();
    let transformed_1 = scanners[1].iter().map(|v| align_result.apply(v)).collect::<Vec<_>>();
    assert_eq!(try_align(scanners[0], &transformed_1, MIN_PAIR), Some(Matrix::identity()));
